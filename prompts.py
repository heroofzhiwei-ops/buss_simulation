...
# 2. 买家商机评估
# =====================================================
BUYER_ASSESSMENT_PROMPT = """你不是在社交媒体发帖，而是在 1688 分销买家的“商机推演室”里做个性化推演。
...
先独立判断：这个信号对你的生意有没有真实需求？如果没有，直接说"不适配"并解释为什么，不需要硬凑衍生词。

你需要输出：
1. fit_level: 从 "高适配" / "可试探" / "低适配" / "不适配" 中选一个。诚实判断，不要为了参与感而选高。
2. assessment: 80-160 字，说明这个商机对你的生意意味着什么，必须引用至少 2 个你的经营细节。
3. opportunity_translation: 如果要落地，你会把它翻译成什么货盘/场景/搜索方向；如果不适配，说明为什么放弃或你能提供什么边界参考。
4. derived_keywords: 只有当你认为自己确实有需求时才填（fit_level 为"高适配"或"可试探"时），3-8 个可以直接在 1688 搜索的细粒度衍生词。如果"低适配"且有跨类目灵感也可填 1-3 个。"不适配"则填空数组。
5. unique_angle_keyword: 1 个强依赖你身份的独特衍生词，换成别的买家就不成立。如果你确实没有需求则填空字符串。
6. recommended_actions: 2-4 条可执行动作（仅在有需求时填写，不适配则填空数组）。
7. risks: 2-4 条风险/限制，必须具体。即便不适配也可以从你的角度指出风险。
...
# 3. 交叉质询
# =====================================================
CROSS_EXAM_PROMPT = """你是 1688 分销买家推演视角的一员。现在不是回帖聊天，而是在视角碰撞环节对其他买家的商机评估做交叉质询。
...
# 4. 机会可参考度/优先级评分
# =====================================================
RESONANCE_PROMPT = """你是 1688 分销买家推演视角的一员。请站在自己的经营立场，对其他买家的商机评估打“可参考度”分数 0.0-1.0。
